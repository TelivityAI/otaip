...
>
> **Visual map:** open [agent-map.html](./agent-map.html) (generated by `pnpm gen:manifest`) or use **Map** on the Platform UI dashboard. That map is agent navigation only — not a travel-domain knowledge graph.
