 ...
 *   so on `import @otaip/agents-booking` (after publish) the bundled
...
if (!DIST_AVAILABLE) {
  ...
  console.warn(
    `[skip] @otaip/agents-booking dist-runtime test: ${DIST_INDEX} not found.\n` +
      '       Run `pnpm --filter @otaip/agents-booking build` first.',
  );
}

describeIfBuilt('@otaip/agents-booking — built bundle loads + invokes', () => {
  ...
  it('exports the agent classes that depend on JSON data', async () => {
    ...
    expect(typeof mod['ApiAbstraction']).toBe('function');
    expect(typeof mod['GdsNdcRouter']).toBe('function');
  });
});
