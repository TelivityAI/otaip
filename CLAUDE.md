...
| "Residual value for a partially flown ticket is the original fare minus the flown portion" | STOP. Passenger residual = Cat 33 + THB (Historical Ticket Based) when filed, else carrier-specific amounts. MPA-P is interline settlement — not pax residual. Never haversine-split a through fare. See `docs/knowledge-base/partial-refund-residual-value.md`. |
